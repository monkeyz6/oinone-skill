---
name: oinone-create-open-api
description: Use when creating oinone-pamirs open interfaces (@Open annotation) that expose internal functionality as REST APIs for external systems to call inbound. Triggered by "创建开放接口", "开放接口", "对外提供接口", "暴露API", "对外暴露", "提供接口给外部", "@Open", "openapi/pamirs", or any scenario where an internal Pamirs function needs to become an HTTP endpoint reachable via the EIP open-api gateway. Activate even when the user does not explicitly say "@Open" or "EIP" — match on intent ("让外部系统调用我们的XX", "给ERP/电商/物流提供接口"). This skill covers ONLY inbound open APIs (external → us); for outbound calls (us → external) use oinone-create-eip-integration instead.
metadata:
  scope: oinone-pamirs / deli-b2b-oversea & kailas-aries-oversea
  version: "1.1"
---
