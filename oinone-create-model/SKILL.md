---
name: oinone-create-model
description: Create or modify Oinone/Pamirs domain models and fields — `@Model` + `@Model.model("ns.ClassName")` Java entity classes, `@Field` typed columns (String/Text/Integer/Boolean/Float/Money/Date/Enum/Binary), relations (`@Field.many2one`/`one2many`/`many2many`/`one2one` + `@Field.Relation`), inheritance (`@Model.MultiTable`/`ChangeTableInherited`), optimistic locking (`VersionModel`), and base classes (`IdModel`, `CodeModel`, `BizCodeModel`, `TransientModel`, `BaseRelation`). Use when the user asks to 创建模型 / 改模型 / 加字段 / 新增实体 / 加关联 / 中间表 / 多对多 / 一对多 / 加索引 / 乐观锁 / 自动编码 / 代理模型 / 抽象模型 / 传输模型 (DTO) — even when they don't explicitly say "model" or "@Field" (e.g. 在订单上加 remark 字段, 给 Company 关联负责人, 做个 DTO). Specific to Oinone/Pamirs. NOT for Spring/JPA entities, NOT for enums (use oinone-create-enum), NOT for Actions on the model (use oinone-create-action), NOT for Hooks (use oinone-create-hook).
license: Proprietary
metadata:
  scope: oinone-pamirs
  version: "2.1"
