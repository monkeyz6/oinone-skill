---
name: oinone-create-action
description: Create or modify Oinone/Pamirs Actions and Functions on a model — `@Action` (ServerAction button), `@UxRouteButton` (page jump / dialog / drawer), `@UxLinkButton` (external URL), `@UxClientButton` (client-side action), and `@Function` (backend method, optionally exposed as GraphQL/REST). Triggered by 服务端动作 / 按钮 / 表单提交 / 批量操作 / 跳转详情 / 跳转新建 / 弹窗按钮 / 外部链接 / 前端校验 / 刷新表格 / GraphQL 接口 / 后端方法 / 业务逻辑函数 / @Action / @Function / ServerAction / ViewAction / API endpoint / button handler / business method.
---
