---
name: oinone-create-eip-integration
description: >
  Generate oinone-pamirs EIP integration interface code using the @Integrate annotation pattern.
  Use this skill whenever the user wants to create an integration interface, connect to an external API,
  call a third-party service, set up EIP configuration, or any outbound HTTP integration with external systems.
  Supports both JSON (default) and XML request/response formats via custom Serializable / InOutConverter.
  Trigger keywords: "创建集成接口", "集成外部接口", "对接XX接口", "调用第三方XX", "EIP接口",
  "集成XX系统", "推送数据到XX", "从XX拉取数据", "@Integrate", "IEipAnnotationSingletonConfig",
  "create integration", "integrate with", "third-party API",
  "XML接口", "XML格式", "XML响应", "解析XML", "application/xml", "text/xml".
  Activate this skill even when the user does not explicitly say "EIP" or "integration",
  provided they describe a need to call any external HTTP API from within the oinone-pamirs framework.
compatibility: Designed for oinone-pamirs (Java 8 + Spring Boot 2.3) projects deli-b2b-oversea and kailas-aries-oversea.
license: Proprietary — internal use only.
metadata:
  author: deli-aries-team
  version: "3.0"
  domain: oinone-pamirs-eip
---
